...
*126 E Burke St
*6
*Drew Holdings, llc... BRIX27, LLC... ABRAHAM ASHTON... HUB CO-OP LLC... RONIN PROPERTIES LLC
```{r}
zip_25401_loan_sample <- read.csv ("data/cleaned_zip_25401_loan_sample.csv")
...
**A2.**
It does not seem weird that they all used the same address. The address provided is a communal workspace operated under the LLC that appears twice ("Hub Co-Op"). The website states that businesses can rent out office spaces at that address.
...
**A3.**
*Martinsburg, WV -- 1165 loans
*Full-Service Restaurants at 49 loans.
```{r}
berkeley_ppp <- 
  wv_ppp %>%
  #I tried uploading the data set to open refine but it appeared corrupted even though it works inside of RStudio.
  filter(str_detect(project_county_name, "berkeley|Berkeley|BERKELEY"))

cleaned_berkeley_ppp <- berkeley_ppp %>%
    mutate(city_clean=key_collision_merge(city)) %>%
      select(id:city, city_clean, everything())

berkeley_city_loans <- cleaned_berkeley_ppp %>%
  group_by(city_clean) %>%
   summarize(
    count=n()
  ) %>%
...
martinsburg_loans <- cleaned_berkeley_ppp %>%
  filter(str_detect(city_clean, "Martinsburg"))

martinsburg_naics <-
  martinsburg_loans %>%
    left_join(naics_codes, by=c("naics_code"="ï..naics_code"))
...
**A4.** 
That either restaurants were hit hardest during the pandemic or that there is a higher number of restaurants in Martinsburg than any other industry.
...
*2021-02-17
*94
*Yes, the entity owns several restaurants. The names are Kitzie's Cafe and Riverbend Bar & Grill.
*Kitzie's Cafe has 3 locations that are open, 2 more have closed in the past few years. Riverbend has 1.
*Kitzie's of Inwood closed on May 19, 2020, three months after they received the loan.