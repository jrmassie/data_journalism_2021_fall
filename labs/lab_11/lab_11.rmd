...
**A1.** 
The middle of the state received the highest number of loans. Presumably this region is more mountainous or rich in minerals. 
Ritchie County had the highest loan amount per 100k at $77,752,742.79. The total loan amount was $7,653,980, 59 total loans and the county has a population of 67,908.
```{r}
wv_mining_industries <- wv_ppp %>%
...
wv_pop_by_county <- get_acs(geography = "county",
  ...
  mutate(COUNTY = str_remove_all(COUNTY, ", West Virginia")) %>%
  mutate(COUNTY = toupper(COUNTY)) %>%
  mutate(COUNTY = str_trim(COUNTY, side = "both")) %>%
  clean_names()
```
```{r}
wv_mining_and_pop <- wv_pop_by_county %>%
  left_join(wv_mining_industries, by=c("county" = "project_county_name")) %>%
...
```{r}
ggplot() +
  geom_sf(data = wv_mining_and_pop, aes(fill = amount_per_100k)) +
  theme_void() +
...
wv_mining_and_pop %>%
  as_tibble() %>%
  select(county, population, total_loans, total_amount, amount_per_100k) %>%
...
**A2.**
Yes, it is a similar pattern with a concentration in the middle of the state.
```{r}
...
operational_mines <- mine_permits %>%
...
ggplot() +
  ...
  geom_sf(data=operational_mines)
```
...
**A3.**
There is only one mine in that county.
```{r}
ggplot() +
  geom_sf(data = wv_mining_and_pop, aes(fill = amount_per_100k)) +
...
**A4.**
The company is HALL DRILLING, LLC which had a total loan amount of $6,866,600. They are in the drilling oil and gas wells industry. They off trucking, drilling, production installation, construction, and roustabout services. According to the data they retained 387 jobs. My guess is that material shortages may have limited their operations or covid protocols may have made the requirements to work safely impossible during the pandemic.
```{r}
ritchie_wv <- wv_ppp %>%
  filter(str_detect(naics_code, "21....")) %>%
