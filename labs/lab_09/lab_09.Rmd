...
april_wv_ppp %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total)) +
  geom_point(aes(x=date_approved, y=total))+
  geom_text(aes(x=date_approved, y=total, label=total), hjust=1, vjust=1)+
  scale_x_date(date_breaks = "1 day", date_labels = "%x") + 
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1)
  ) +
  labs(
    title="West Virginia April PPP loans",
    x = "date",
    y = "total loans",
...
**A2.** 
The top 10 states are, in order, Nebraska, Georgia, Iowa, Illinois, Louisiana, Florida, Mississippi, Montana, Wyoming, Kansas.
```{r}
...
```
```{r}
total_state <- get_acs(geography='state', variables=c(total_population = 'B01001_001'))
```
```{r}
loans_per_100k <- state_ppp_all %>%
  inner_join(total_state, by=c("state"="NAME"))

loans_per_100k <- loans_per_100k %>%
...
loans_per_100k %>%
  ...
  labs(
    title="Top 10 PPP Loans by State Per 100k",
    x="State",
    Y="PPP Loans Per 100k",
    caption="Sources: SBA PPP loan data and U.S. Census data"
  )
```
...
**A3.**  
I believe this means that the vast majority of people receiving under $25k had their loans forgiven. Many people had parts of their loans forgiven, and very few had none. 
```{r}
wv_ppp_loans <- read_rds("data/lab_09.rds")
```

```{r}
wv_ppp_under25k <- wv_ppp_loans %>%
  filter(amount < 25000) %>%
