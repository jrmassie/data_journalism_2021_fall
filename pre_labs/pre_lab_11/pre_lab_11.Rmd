...
**Answer**
There are multiple location data columns.
```{r}
...
**Answer**
Now the data is only showing Maryland hospitals.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It's a breakdown of types of hospitals. We have mostly General Acute Care hospitals here in Maryland.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It's similar to the data displayed before, but now only with general acute care hospitals.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It's a scatter plot with longitude and latitude on the Y and X axises.
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It said "To enable caching of data, set `options(tigris_use_cache = TRUE)` in your R script or .Rprofile."
So I copy and pasted the command to run it in the console and enable the caching.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
more location data with 3,233 variables.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Created a variable with just the 24 Maryland counties.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It looks like the state of Maryland with county lines drawn as well, minus the rivers.
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Same as before but now with the hospitals points layered over top.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The counties are now white and dots are colored to corospond with the number of beds the hospital offers.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A data frame with different hospitals.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Breakdown of every county and total number of beds.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Each county with population data in alphabetical order.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The county data again but with geometry data, latitude and longitude.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The county population table and the number of hospital beds.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
MD counties with population data and geometric data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Same table as before but with the added total_beds column.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now a proportion of beds per 100k people is displayed. Baltimore City stays high but, surprisingly, counties with lower populations like Allegany and Somerset rise up the list when arranged by beds proportionally.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The same data visualized on a map of Maryland with a blue gradiant to shot beds per 100k.
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The color is now switched to ultraviolet shades.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
A lot of the middle points for beds are in the middle of the state, presumably between population centers. Western Maryland has a very high number of beds per capita, and Southern Maryland has the lowest number of beds of any region.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
