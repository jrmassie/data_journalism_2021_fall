...
**Answer**
<dbl> or numbers, <chr> or characters, <dbl> or numbers
```{r}
...
**Answer**:
135073; It is all the same amount. You can conclude that there is a significant chunk of the data that has either not recieved forgiveness or that data has not been entered correctly.
```{r}
ppp_maryland_loans %>%
...
**Answer**
The largest amount is 10000000 and the smallest is 6. The 6 stands out to me more because I believe 10000000 is the maximum you could request. 6 is such a small amount for a loan, I would at the very least be interested in who is requesting that loan and why.
```{r}
...
**Answer**
The City Casing is inconsistent, the amount are characters, and so are the zips.
```{r}
arnold_md_loans
```
...
**Answer**
Problem with `summarise()` column `total_amount`. i `total_amount = sum(amount)`. x invalid 'type' (character) of argument
```{r}
# cleaning function
