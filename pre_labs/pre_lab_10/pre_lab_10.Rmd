...
It's a table with each county and the number of loans. The code groups the data by county and then counts the number of loans and displays it.
...
**Answer** Describe what the code below is doing and the output.
The code is the same except it is filtering for all counties with more than 6 loans.
...
They are 5-digit unique identifiers in each county's row.
...
Now the original data frame is combined with the fips code frame.
```{r}
...
Now I only see the MD counties.
...
**Answer** Describe the changes from the previous map
Now it displays the county name and added a comma to the number of loans.
...
https://datawrapper.dwcdn.net/qooyb/1/
