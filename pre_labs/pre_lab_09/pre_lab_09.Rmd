...
**Answer**
Line 1 tells R to store the resulting code in the variable and what data set we are using. Line 2 tells R to group the data by the column "project_county_name." Line 3 tells R to summarize the data. Line 4 tells it how, by the sum of the loans in each county. Line 5 tells it that more will be done to the data. Line 6 tells R to present the data in descending order based on the total loans. Line 7 tells R to stop the data set after 10 results. And Line 9 tells R to show the resulting table stored in the variable.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
I see the data stored in the variable in a gray bar chart.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
I see the same data, now reordered in ascending order.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The chart is now flipped and the county names are visible.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
It removed the gray background.            
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer**
now there is a blue background.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
There is a title and the label on they axis is more legible.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see a new data set that counts the total loans per approval date.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
There are now more ticks on the X-axis.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
