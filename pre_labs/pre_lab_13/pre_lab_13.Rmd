...
**Answer**
Each state with who they went for, percentage, and money recieved.
```{r}
...
**Answer**
It's a scatter plot with not much consitency. 
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
Another scatterplot that's more level than the last.
```{r}
...
**Answer**
There's just two columns, showing the pct for trump and businesses receiving ppe funds.
```{r}
reveal_data %>%
...
**Answer**
There's a lot of statistics terms and values like confidence interval and p-value.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
The r value is smaller and the p-value is larger.
```{r}
...
**Answer**
Both mean ppp amount and mean businesses receiving ppp funds voted for Trump over Clinton.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
It's similar to the previous output but with means now instead of an r-value displayed.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
The same table as before. The p-value is significantly lower. 
```{r}
