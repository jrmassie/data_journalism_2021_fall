...
**Answer**
It's a list of Tweets with unique ids and other information on the person making the tweet.
```{r}
...
**Answer**
I combined the same words with the same casing, but not the ones with different casing. 
```{r}
...
**Answer**
There's a list of 5369025 words that are every word used in a tweet.
```{r}
...
**Answer**
The graph mostly shows common words like "the" or "to." The most interesting thing is seeing covid show up more than once in different forms in these words used in every sentence, regardless of content.
...
**Answer**
Much more interesting, it shows how much the pandemic took over our communication.
```{r}
unique_words %>%
...
**Answer**
Now there is a breakdown between words used in 2020 versus 2021. Both have a lot of the same words.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
covid 19 is far and away the most used phrase and then there's a mix of other pandemic terms around the same usage. 
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Same as before, with covid 19 extending its lead on the group, but now you see vaccine creep up as well.
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
The biggest difference is that health care and public health show up for Democrats, american people for Republicans.
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
They are different. The first table shows President Trump and second Joe Biden for instance. 
```{r}
covid_tweets %>%
...
**Answer**
Some of the words depend on how you use them to determine if they are positive or negative, particularly proper nouns like someone's name.
```{r}
