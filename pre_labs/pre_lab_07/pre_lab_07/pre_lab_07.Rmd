...
**Answer**
I see an overview of the industry classification codes and some tables explaining what the codes mean.
### Task 3: Load image
...
**Answer**
I see the HTML source code of the webpage.
...
**Answer**
It's a nested list that says HTML at the top and breadks out into the different sections of an HTML page (head and body). There is a seperte tab that says "xml attributes."
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
I see the code for the table nested under the subhead <thead>.
### Task 9: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
```{r, echo=FALSE}
...
**Answer**
I see all of the tables on the webpage displayed below the codeblock.
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
All of the tables from the webpage are in a numbered list 1-6.
...
**Answer**
A new tab with the first table opened up, titled "naics_industry[[1]].
### Task 14: Load image
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
Same thing is displayed, just with the second table.
### Task 16: Load image
...
**Answer**
The second table with 2 columns and 21 rows is displayed. 
```{r}
...
**Answer**
The below data has a description attatched to each sector, saying what it is.
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
I see a page for Utilities NAICS 22
...
**Answer**
I see some html code for the table we want.
```{r}
...
**Answer**
It can't run because jun_2021 doesn't exist as a column. Changing it to Aug to better see. After doing so, now I see a table output. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
I see a NAICS page on Mining, Quarrying, and Oil and Gas Extraction.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
I see a NAICS page for Construction.
...
**Answer**
I see each sector written out below.
```{r}
...
**Answer**
Nothing is displayed below the code block, but a new value called list_of_sectors was created.
```{r}
...
**Answer**
I see the same list printed out as before.
```{r}
...
**Answer**
R couldn't find any object called "sector_list."
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
The same list of sectors as before.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
R can't find an object called "sector_name."
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**
I see a table with a sector number and description.
```{r}
...
**Answer**
I see 20 which is the number of rows in the table.
```{r}
nrow(naics_industry)
```
...
**Answer**
A list of numbers counting from 1 to 20.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
Again I changed it from jun to aug. I see the same table as before, showing the data_series and month_year.
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
I see the sector number and corresponding description of different fields.
```{r}
# Define url of page we want to scrape
...
**Answer**
Below is the same table as before.
```{r}
...
**Answer**
The same table with a sector_url column and sector_xpath_id column.
```{r}
...
**Answer**
The same table as before just without the Public Administration sector.
```{r}
...
**Answer**
I each sector printed into it's own seperate table.
```{r}
...
**Answer**
I see the urls for each row and the xpath on the end, completing them.
```{r}
...
**Answer**
I see all of the data tables from the htmls.
```{r}
...
**Answer**
The same data as before but now with just the "Employment, all employees (seasonally adjusted)" row.
```{r}
...
**Answer**
Same table, now with the data from teh sector tables added on.
```{r}
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
Again replacing jun with aug. Now the only columns are the sector number, description, and aug_2021 for each sector.
```{r}
...
for(row_number in 1:nrow(naics_industry)) {
    ...
    employment_info <- employment_info[[1]] %>%
      ...
      slice(2) %>%
      bind_cols(each_row_df) %>%
      mutate(aug_2021 = parse_number(aug_2021)) %>%
      rename(aug_2021_employees = aug_2021) %>%
      select(sector,description,aug_2021_employees)
...
**Answer**
Now all of the dataframes are combined.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
The Agriculture, Forestry, Fishing and Hunting NAICS page.
### Task 39: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Answer**
Now the value for agriculture says "na."
```{r}
# remove the suspicious value for agriculture.
employment_by_sector_all <- employment_by_sector_all %>%
  mutate(aug_2021_employees = na_if(aug_2021_employees,5.7))
