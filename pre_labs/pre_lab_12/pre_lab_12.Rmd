...
**Answer**
A big list of vaccinations by county. 
```{r}
...
**Answer**
A two column table with the county name and number of times the county shows up on the original table.
...
**Answer**
It could be data that was put in wrong or had some sort of recording error.
...
**Answer**
The past 7 days saw a spike in full vaccinations in the past few days. 
```{r}
...
**Answer**
There's a large spike in the spring of 2021.
```{r fig.width=9}
ggplot() +
...
**Answer**
The number of people in Prince George's County fully vaccinated on December 02 NA by NA percent compared to the week before
```{r}
...
**Answer**
A line chart with just Maryland counties.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
It shows the Maryland counties' cases over time. The cases spike in the begining and then start to consistently raise.
```{r}
...
**Answer**
The map is showing the most up to date number of coronavirus cases by county.
```{r}
ggplot() +
